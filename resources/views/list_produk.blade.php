                ...
                <tr>
                    ...
                    <td class="text-center bg-gray-100">
                        <form action="{{ route('produk.delete', $id[$index]) }}" method="POST">
                            @csrf
                            @method ('DELETE')
                            <button class="font-medium text-red-600 dark:text-red-500 hover:underline" style="font-weight: sans-serif;" type="submit" 
                            onclick="return confirm('Are you sure want to delete {{ $item }}?')">Delete</button>
                        </form>
                    </td>
                </tr>
                @endforeach
